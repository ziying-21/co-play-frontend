...
const Singlestory = () => {
  ...
  const role1 = new RoleAgent(0, "示例2", 19, "男");
  const role2 = new RoleAgent(0, "示例3", 20, "男");
  const role3 = new RoleAgent(0, "示例4", 18, "女");
  const role4 = new RoleAgent(0, "示例5", 21, "女");
  const role5 = new RoleAgent(0, "示例6", 23, "男");
  const role6 = new RoleAgent(0, "示例7", 17, "男");
  const scene0 = new SceneAgent(0, "示例1", "示例", "示例", ["示例"], "示例");
  const scene1 = new SceneAgent(0, "示例2", "示例", "示例", ["示例"], "示例");
  const scene2 = new SceneAgent(0, "示例3", "示例", "示例", ["示例"], "示例");
  const scene3 = new SceneAgent(0, "示例4", "示例", "示例", ["示例"], "示例");

  const interaction0 = new Interaction(0, 0, "movement", "我打你");
  const interaction1 = new Interaction(0, 0, "emotion", "心理活动很复杂");
  const interaction2 = new Interaction(0, 0, "expression", "脸上露出不悦的神情");
  const interaction3 = new Interaction(0, 0, "speech", "我说：你好");
  const interaction4 = new Interaction(0, 0, "speech", "她说：你好");
  const interaction5 = new Interaction(0, 0, "movement", "我打死你");
